...
pub fn main<T: Plugin + Default>(callback: HostCallbackProc) -> *mut AEffect {
    ...
    //     // let mut aeffect_ptr = std::mem::MaybeUninit::<AEffect>::zeroed();
    //     // aeffect_ptr.as_mut_ptr()
    //     //   const AEFFECT_SIZE: usize = mem::size_of::<AEffect>();
    //     //   ////mem::zeroed::<AEffect>()
    //     //   let p = mem::zeroed::<[u8; AEFFECT_SIZE]>();
    //     //   //let p: *const [u8; std::mem::size_of::<AEffect>()] =
    //     //   //slice.as_ptr() as *const [u8; std::mem::size_of::<AEffect>()];
